...
def compress_image(msg, output):
    ...
    if cv2_img.shape[0] > 200 or cv2_img.shape[1] > 200:
        stride = int(np.ceil(max(cv2_img.shape[0] / 200.0, cv2_img.shape[1] / 200.0)))
...
def compress_point_cloud2(msg, output):
    ...
    if np.little_endian:
        ...
        points_uint16 = np.stack((xpoints_uint16, ypoints_uint16, zpoints_uint16), 1).ravel().byteswap().view(
            dtype=np.uint8)
